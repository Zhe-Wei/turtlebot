...
class laserTracker:
	def __init__(self):
		...
		# parameters for the noise filter
		self.winSize = rospy.get_param('~winSize', 2)
		self.deltaDist = rospy.get_param('~deltaDist', 0.2)

		# parameters for the robot name and the scan topic
		self.robot_name = rospy.get_param('~robot_name', 'robot_1')
		self.scanTopic = rospy.get_param('~scan_topic', 'scan')

		# subscribers and publishers
		self.scanSubscriber = rospy.Subscriber(self.scanTopic, LaserScan, self.registerScan)
	...
	def registerScan(self, scan_data):
		...
		if(minDistance > scan_data.range_max):
			...
			if tmpAngle != minDistanceAngle and tmpDistance != minDistance:
				tmpAngle = minDistanceAngle
				tmpDistance = minDistance
				# alignment text formate and round 2 digits
				# rospy.logwarn('laser angle: {0:.2f} distance: {1:.2f}'.format(minDistanceAngle, minDistance))
				# print("angle: {0:.2f} distance: {1:.2f}".format(minDistanceAngle, minDistance))
...
if __name__ == '__main__':
	...
	rospy.loginfo('Starting Laser Tracker node...')


	while not rospy.is_shutdown():
		try:
			tracker = laserTracker()
			# Start ROS main loop and wait until the node is stopped
		except rospy.ROSInterruptException:
			# Log an exception if the node is interrupted
			rospy.logwarn('Laser Tracker node has been interrupted.')
