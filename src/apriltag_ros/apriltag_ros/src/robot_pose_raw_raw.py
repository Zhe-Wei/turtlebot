...
map2odom = PoseWithCovarianceStamped()
map2odom_pub = rospy.Publisher('turtlebot02/map2odom', PoseWithCovarianceStamped, queue_size=10)
...
def main():
    rospy.Subscriber("turtlebot02/tag_detections", AprilTagDetectionArray, apriltag_callback, queue_size = 1)
...
def apriltag_callback(data):
    ...
    if data.detections:
        ...
        for detection in data.detections:
            # tag_id = str(detection.id)  # tag id
            # tag_id = tag_id[1:2]
            bundle_first_tag_id = str(detection.id[0])
            # print(type(tag_id))
            rospy.logdebug("Tag ID detected: %s \n", bundle_first_tag_id)
            
            if bundle_first_tag_id == "5":
                bundle_first_tag_id = "1"
            elif bundle_first_tag_id == "10":
                bundle_first_tag_id = "2"
            elif bundle_first_tag_id == "15":
                bundle_first_tag_id = "3"
            elif bundle_first_tag_id == "20":
                bundle_first_tag_id = "4"
            elif bundle_first_tag_id == "25":
                bundle_first_tag_id = "5"
            elif bundle_first_tag_id == "30":
                bundle_first_tag_id = "6"
            elif bundle_first_tag_id == "35":
                bundle_first_tag_id = "7"
            elif bundle_first_tag_id == "40":
                bundle_first_tag_id = "8"
            elif bundle_first_tag_id == "45":
                bundle_first_tag_id = "9"
            elif bundle_first_tag_id == "50":
                bundle_first_tag_id = "10"
            elif bundle_first_tag_id == "55":
                bundle_first_tag_id = "11"
            elif bundle_first_tag_id == "60":
                bundle_first_tag_id = "12"
            elif bundle_first_tag_id == "65":
                bundle_first_tag_id = "13"
            elif bundle_first_tag_id == "70":
                bundle_first_tag_id = "14"
            elif bundle_first_tag_id == "75":
                bundle_first_tag_id = "15"
            elif bundle_first_tag_id == "80":
                bundle_first_tag_id = "16"
            elif bundle_first_tag_id == "85":
                bundle_first_tag_id = "17"
            elif bundle_first_tag_id == "90":
                bundle_first_tag_id = "18"
            elif bundle_first_tag_id == "95":
                bundle_first_tag_id = "19"
            # print(tag_id)
            child_frame_id = "bundle_" + str(bundle_first_tag_id)

            # Convert the deteced tag Pose object to tag pose representation as a list, only for convinience
            poselist_tag_wrt_camera = pose2poselist(detection.pose.pose)
...
def broadcast_last_transform():
        ...
        if new_translation:
            ...
            ts_odom_wrt_map.transform.translation.x = new_translation[0][0]+0.07
            ...
            print(round(ts_odom_wrt_map.transform.translation.x, 5), round(ts_odom_wrt_map.transform.translation.y, 5), round(ts_odom_wrt_map.transform.translation.z, 5))
            map2odom.header.stamp = rospy.Time.now()
            map2odom.header.frame_id = "map"

            map2odom.pose.pose.position.x = ts_odom_wrt_map.transform.translation.x
            map2odom.pose.pose.position.y = ts_odom_wrt_map.transform.translation.y
            map2odom.pose.pose.position.z = ts_odom_wrt_map.transform.translation.z

            map2odom.pose.pose.orientation.x = ts_odom_wrt_map.transform.rotation.x
            map2odom.pose.pose.orientation.y = ts_odom_wrt_map.transform.rotation.y
            map2odom.pose.pose.orientation.z = ts_odom_wrt_map.transform.rotation.z
            map2odom.pose.pose.orientation.w = ts_odom_wrt_map.transform.rotation.w
            try:
                br_odom_wrt_map.sendTransform(ts_odom_wrt_map)
                map2odom_pub.publish(map2odom)
