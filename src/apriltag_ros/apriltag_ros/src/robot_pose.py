...
map2odom = PoseStamped()
map2odom_pub = rospy.Publisher('turtlebot02/map2odom', PoseStamped, queue_size=10)
...
def transformPose(pose, sourceFrame, target_frame):
    ...
    '''
    # print(sourceFrame, target_frame)
    _pose = PoseStamped()
...
def matrix_from_xyzquat_np_array(arg1, arg2=None):
    if arg2 is not None:
        ...
        translate = arg1[0:3]
        quaternion = arg1[3:7]
        # print("translate: ",translate,"quaternion: ",quaternion,"\n")
        # print(np.dot(compose_matrix(translate=translate),quaternion_matrix(quaternion)))
...
def apriltag_callback(data):
    ...
    if data.detections:
        ...
        for detection in data.detections:
            ...
            # 調整誤差
            detection.pose.pose.pose.position.x = (2/3)*(detection.pose.pose.pose.position.x) # 水平方向
            detection.pose.pose.pose.position.z = (5/7)*(detection.pose.pose.pose.position.z)-0.01 # 前後方向
            ...
            # print(str(tag_id),":",poselist_base_wrt_tag)
...
def map_to_odom_transform():
        ...
        # robot_pose = map->base      (translation, rotation) = base->map
        (translation, rotation) = convert_pose_inverse_transform(robot_pose)
        # print(translation)
        # print("robot_pose", robot_pose.pose.position.x, robot_pose.pose.position.y, robot_pose.pose.position.z)
        ...
        # print("map_wrt_base_pose", map_wrt_base_pose.pose.position.x, map_wrt_base_pose.pose.position.y, map_wrt_base_pose.pose.position.z)
        ...
        # print("ts_odom_wrt_base", ts_odom_wrt_base.transform.translation.x, ts_odom_wrt_base.transform.translation.y, ts_odom_wrt_base.transform.translation.z)
        map_wrt_odom_pose = tf2_geometry_msgs.do_transform_pose(map_wrt_base_pose, ts_odom_wrt_base) ## 這個出來的值有錯
        # print("map_wrt_odom_pose", map_wrt_odom_pose.pose.position.x, map_wrt_odom_pose.pose.position.y, map_wrt_odom_pose.pose.position.z)
...
def broadcast_last_transform():
        ...
        if new_translation:
            ...
            ts_odom_wrt_map.transform.translation.x = new_translation[0][0]+0.1
            ...
            # map2odom.pose.position.x = new_translation[0][0]+0.1
            # map2odom.pose.position.y = new_translation[1][0]
            # map2odom.pose.position.z = new_translation[2][0]

            # map2odom.pose.orientation.x = new_rotation[0]
            # map2odom.pose.orientation.y = new_rotation[1]
            # map2odom.pose.orientation.z = new_rotation[2]
            # map2odom.pose.orientation.w = new_rotation[3]
            try:
                # print(ts_odom_wrt_map.transform.translation.x,  ts_odom_wrt_map.transform.translation.y, ts_odom_wrt_map.transform.translation.z)
                # print(map2odom.pose.position.x,map2odom.pose.position.y,map2odom.pose.position.z)
