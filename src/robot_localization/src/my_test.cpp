...
void OdemCallback(const geometry_msgs::Pose::ConstPtr &msg)
{
	...
	for(unsigned int num = 0; num < q_d.size(); num++) {
		switch(num) {
			case 0:    // first downward pose sensor
				...
				Calibration_d1.header.frame_id = "turtlebot02/odometry_sensor1";
			...
			case 1:   // second downward pose sensor
				...
				Calibration_d2.header.frame_id = "turtlebot02/odometry_sensor1";
			...
			case 2:   // third downward pose sensor
				...
				Calibration_d3.header.frame_id = "turtlebot02/odometry_sensor1";
...
int main(int argc, char** argv)
{
	...
	// ros::Publisher Pose_d etect_downward3;
	...
	// Variable setting
	// Pose_detect_downward1 = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/turtlebot02/pose_d1", 100);         
	Pose_detect_forward1 = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/turtlebot02/pose_f1", 100);
	// Pose_detect_downward2 = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/turtlebot02/pose_d2", 100);
	Pose_detect_forward2 = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/turtlebot02/pose_f2", 100);
	// Pose_detect_downward3 = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/turtlebot02/pose_d3", 100);
	// Pose_detect_forward3 = nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("/turtlebot02/pose_f3", 100);

	forward_tag = nh.subscribe("/turtlebot02/tag_detections", 100, forCallback);
	// downward_tag = nh.subscribe("/downward/tag_detections", 100, downCallback);
	ode_sub = nh.subscribe("/turtlebot02/odom/pose/pose", 100, OdemCallback);
